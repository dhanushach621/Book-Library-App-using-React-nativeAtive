...
export default function Search({ route, navigation }) {
    const { allBooks } = route.params;

    const [filteredBooks , setFilteredBooks ] = useState(allBooks )
    const handleSearch = (value) => {
        console.log(value);
        let vallue2 = allBooks.filter((item) => (item.name).toLowerCase().includes(value.toLowerCase()))
        setFilteredBooks(vallue2)
        //console.log(vallue2);
    }
    return (
        <>
                        ...
                        <View style={styles.MainDataInside}>
                            <ScrollView>
                                ...
                                {filteredBooks?.map((item, index) => {
                                    return (
                                        <TouchableOpacity activeOpacity={0.8} style={styles.singleData} onPress={() => navigation.navigate('SingleView', { item: item, allBooks: allBooks })}>
                                            ...
                                            <View>
                                                ...
                                                <View style={styles.Actions}>
                                                    ...
                                                    <Text style={styles.BooksName}>    by {item.author}</Text>
